...
@section('title', __('webCaption.menu_group_edit.title'))
@else
@section('title', __('webCaption.menu_group_add.title'))
@endif
...
<form action="{{ route('menu-groups.store')}}" method="POST">
	...
	<div class="card card-primary">
		<div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.menu_group_edit.title')}}
			@else
			{{__('webCaption.menu_group_add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<x-admin.form.inputs.text tooltip="{{__('webCaption.order.caption')}}" label="{{__('webCaption.order.title')}}"  for="order" class="form-control" name="order"  placeholder="{{__('webCaption.order.title')}}" value="{{old('order', isset($data->order)?$data->order:'' )}}"  required="required" />
							@if ($errors->has('order'))
					...
					<div class="col-md-6">
						<div class="form-group">
							<x-admin.form.inputs.text tooltip="{{__('webCaption.slug.caption')}}" label="{{__('webCaption.slug.title')}}"  maxlength="250" for="slug" class="form-control" name="slug"  placeholder="{{__('webCaption.slug.title')}}" value="{{old('slug', isset($data->slug)?$data->slug:'' )}}"  required="required" />
							@if ($errors->has('slug'))
