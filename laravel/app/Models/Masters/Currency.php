...
class Currency extends Model
{
    ...
    protected $fillable = ['name','title_languages','display', 'parent_id', 'currency_symbol', 'title'];
  ...
  //each category might have multiple children
    ...
    public function scopeKeywordFilter($query, $keyword)
    {
        return $query->where( function($query) use ($keyword) {
            $query->where('name', 'like', '%'.$keyword.'%')->orwhere('currency_symbol','like','%'.$keyword.'%')->orwhere('title','like','%'.$keyword.'%')->orWhereHas( 'children', function($query) use ($keyword) {
