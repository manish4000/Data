...
@section('title', __('webCaption.main_category.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.main_category.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.spare-parts.main-category.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.main_category.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.main_category.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-dash.form.inputs.text tooltip="{{__('webCaption.main_category.caption')}}" label="{{__('webCaption.main_category.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.main_category.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
