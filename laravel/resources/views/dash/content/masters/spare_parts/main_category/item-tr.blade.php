...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_15 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-dash.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('dashmasters.spare-parts.main-category.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('dashmasters.spare-parts.main-category.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-dash.form.buttons.edit href="{{ route('dashmasters.spare-parts.main-category.edit', $item->id) }}" />
        @endif
        &nbsp;
        ...
        @if (Auth::guard('dash')->user()->can('masters-spare-parts-main-category-delete'))
            <x-dash.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('dashmasters.spare-parts.main-category.delete')}}" action="{{route('dashmasters.spare-parts.main-category.delete',$item->id)}}" /> 
        @endif
    </div>           
</div> 