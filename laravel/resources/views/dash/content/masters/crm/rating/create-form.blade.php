...
@section('title', __('webCaption.rating.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.rating.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.crm.rating.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.rating.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.rating.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
