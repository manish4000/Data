...
@section('title', __('webCaption.courier.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.courier.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.erp.courier.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.courier.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.courier.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.courier.caption')}}"
                                label="{{__('webCaption.courier.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.courier.title') }}"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <x-dash.form.inputs.text for="url" tooltip="{{__('webCaption.url.caption')}}"
                                label="{{__('webCaption.url.title')}}" name="url" maxlength="100"
                                placeholder="{{__('webCaption.url.title')}}"
                                value="{{old('url', isset($data->url)?$data->url:'' )}}" />
                        </div>
                    </div>

                    @php
                    $old_default = session()->getOldInput('default');
                    $default = (isset($old_default) && $old_default == 1 ) ? 'checked' : ((isset($data->default) &&
                    $data->default == 1)? 'checked' :'' );
                    @endphp
                    <div class="col-md-4 col-4">
                        <x-dash.form.label for="" value="{{__('webCaption.default.caption')}}" class="" />
                        <div class="form-group">
                            <x-dash.form.inputs.checkbox name="default" for="Yes"
                                label="{{__('webCaption.yes.caption')}}" checked="{{$default}}" value="1"
