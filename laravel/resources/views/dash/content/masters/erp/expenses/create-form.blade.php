...
@section('title', __('webCaption.expenses.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.expenses.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.erp.expenses.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.expenses.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.expenses.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-dash.form.inputs.text tooltip="{{__('webCaption.expenses.caption')}}" label="{{__('webCaption.expenses.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.expenses.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
