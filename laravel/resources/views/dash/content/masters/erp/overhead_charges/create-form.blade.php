...
@section('title', __('webCaption.overhead_charges.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.overhead_charges.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.erp.overhead-charges.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.overhead_charges.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.overhead_charges.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.overhead_charges.caption')}}"
                                label="{{__('webCaption.overhead_charges.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.overhead_charges.title') }}"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <x-dash.form.inputs.text  for="overhead_charges_amount"  tooltip="{{__('webCaption.overhead_charges_amount.caption')}}"  maxlength="20" label="{{__('webCaption.overhead_charges_amount.title')}}" name="overhead_charges_amount"  placeholder="{{__('webCaption.overhead_charges_amount.title')}}" value="{{old('overhead_charges_amount', isset($data->overhead_charges_amount)?$data->overhead_charges_amount:'' )}}" />
                        </div>
                    </div>
