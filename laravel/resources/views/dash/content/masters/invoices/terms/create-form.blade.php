...
@section('title', __('webCaption.terms.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.terms.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.invoices.terms.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.terms.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.terms.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.terms.caption')}}"
                                label="{{__('webCaption.terms.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.terms.title') }}"
