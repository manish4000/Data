...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_44 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.terms.title')}}</div>
    ...
    <div class="make_col width_15 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-dash.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('dashmasters.invoices.terms.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('dashmasters.invoices.terms.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-dash.form.buttons.edit href="{{ route('dashmasters.invoices.terms.edit', $item->id) }}" />
        @endif
        &nbsp;
        ...
        @if (Auth::guard('dash')->user()->can('masters-invoices-terms-delete'))
            <x-dash.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('dashmasters.invoices.terms.delete')}}" action="{{route('dashmasters.invoices.terms.delete',$item->id)}}" /> 
        @endif
    </div>           
</div> 