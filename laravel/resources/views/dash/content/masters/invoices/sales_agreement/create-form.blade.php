...
@section('title', __('webCaption.sales_agreement.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.sales_agreement.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.invoices.sales-agreement.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.sales_agreement.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.sales_agreement.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.sales_agreement.caption')}}"
                                label="{{__('webCaption.sales_agreement.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.sales_agreement.title') }}"
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <x-dash.form.inputs.textarea  for="description"  tooltip="{{__('webCaption.description.caption')}}" label="{{__('webCaption.description.title')}}" maxlength="500" name="description"  placeholder="{{__('webCaption.description.title')}}" value="{{old('description', isset($data->description)?$data->description:'' )}}" />
                        </div>
                    </div>
                </div>
            </div>
