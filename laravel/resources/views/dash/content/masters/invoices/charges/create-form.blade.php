...
@section('title', __('webCaption.charges.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.charges.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.invoices.charges.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.charges.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.charges.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.charges.caption')}}"
                                label="{{__('webCaption.charges.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.charges.title') }}"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <x-dash.form.inputs.text for="charge_amount" maxlength="20"
                                tooltip="{{__('webCaption.charge_amount.caption')}}"
                                label="{{__('webCaption.charge_amount.title')}}" name="charge_amount"
                                placeholder="{{__('webCaption.charge_amount.title')}}"
                                value="{{old('charge_amount', isset($data->charge_amount)?$data->charge_amount:'' )}}" />
                        </div>
                    </div>
                </div>
            </div>
