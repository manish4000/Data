...
@section('title', __('webCaption.inspection.title'). ' ' .__('webCaption.edit.title') )
@else
@section('title', __('webCaption.inspection.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('dashmasters.inspection.store')}}" method="POST">
    @csrf
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    ...
                    @if(isset($data->id) && !empty($data->id))
                    {{__('webCaption.inspection.title'). ' ' .__('webCaption.edit.title')  }}
                    @else
                    {{  __('webCaption.inspection.title').' '. __('webCaption.add.title')}}
                    @endif
                </h4>
            </div>
            <hr class="m-0 p-0">
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="form-group">
                            <x-dash.form.inputs.text tooltip="{{__('webCaption.inspection.caption')}}"
                                label="{{__('webCaption.inspection.title')}}" maxlength="50" for="name" name="name"
                                placeholder="{{ __('webCaption.inspection.title') }}"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <x-dash.form.inputs.text for="inspection_charge" maxlength="20"
                                tooltip="{{__('webCaption.inspection_charge.caption')}}"
                                label="{{__('webCaption.inspection_charge.title')}}" name="inspection_charge"
                                placeholder="{{__('webCaption.inspection_charge.title')}}"
                                value="{{old('inspection_charge', isset($data->inspection_charge)?$data->inspection_charge:'' )}}" />
                        </div>
                    </div>
                </div>
            </div>
