                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="px-2 my-2">
                            ...
                            <x-dash.form.buttons.multipleDelete url="{{route('dashaccounts.payments.delete-multiple')}}" />
                        </div>
                        @endif
                        <table class="table">
                            <thead>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    {{-- <td>{{ $member->status }}</td> --}}
                                    <td>
                                        <x-dash.form.inputs.listing_checkbox id="list{{$member->id}}"  onclick="changeDisplayStatus('{{$member->id}}','{{route('dashaccounts.payments.update-status')}}')"  dataItemId="{{$member->id}}" dataUrl="{{route('dashaccounts.payments.update-status')}}" 
                                            value="{{$member->id}}" checked="{{($member->status == 'Active')? 'checked' :''}}"  /> 
                                    </td>
                                    <td>
                                        <x-dash.form.buttons.edit href="{{ route('dashaccounts.payments.edit', $member->id) }}" />
                                        &nbsp
                                        <x-dash.form.buttons.delete id="{{$member->id}}" name="{{$member->name}}"
                                            url="{{route('dashaccounts.payments.delete')}}"
                                            action="{{route('dashaccounts.payments.delete', $member->id) }}" />
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
