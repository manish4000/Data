...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.vehicle.transmission.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.vehicle.transmission.update-status')}}" 
