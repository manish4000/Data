    ...
    <!-- filter  -->
      <div class="card">
        ...
        <div class="card-body pt-75 pb-75 px-50">
          <form method="GET" action="{{route('masters.vehicle.vehicle-status.index')}}" style="margin-bottom:0px !important;">
            <div class="row">
                ...
                <div class="col-md-12 pt-0 text-center">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.vehicle.vehicle-status.index')}}" />
                </div>
            </div>
          </form>
        </div>
        ...
        <!-- Basic Tables start -->
          ...
          @can('main-navigation-masters-vehicle-vehicle-status') 
                @if(count($data) > 0 )
                    @if (Auth::guard('web')->user()->can('main-navigation-masters-vehicle-vehicle-status-delete'))		
                        {{ $data->onEachSide(1)->links('vendor.pagination.bootstrap-4',['multiple_delete_url' => route('masters.vehicle.vehicle-status.delete-multiple') ] ) }}  
                    @else
          ...
          <div class="main_table mb-2" id="master-list">
            ...
            <x-admin.table.table-heading :headingFields="$heading_array"/>                
                ...
                @foreach($data as $item)
                    @include('content.admin.masters.vechiles.vehicle_status.item-tr', ['item'=>$item])    
                        @if(true || request()->input('search.parentOnlyShowAll') == 1)
                          @foreach($item->children as $childItem)
                             @include('content.admin.masters.vechiles.vehicle_status.item-tr', ['item'=>$childItem])    
                          @endforeach                                        
                        @endif
                @endforeach      
