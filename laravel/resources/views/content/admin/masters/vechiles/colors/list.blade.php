    ...
    <!-- filter  -->
      <div class="card">
        ...
        <div class="card-body pt-75 pb-75 px-50">
          <form method="GET" action="{{route('masters.vehicle.color.index')}}" style="margin-bottom:0px !important;">
            <div class="row">
        ...
        <!-- Basic Tables start -->
          ...
          @can('main-navigation-masters-vehicle-color') 
                @if(count($data) > 0 )
                    @if (Auth::guard('web')->user()->can('main-navigation-masters-vehicle-color-delete'))		
                        {{ $data->onEachSide(1)->links('vendor.pagination.bootstrap-4',['multiple_delete_url' => route('masters.vehicle.color.delete-multiple') ] ) }}  
                    @else
