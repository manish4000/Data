...
<section>
    ...
    <div class="card">
        ...
        <div class="card-body pt-75 pb-75 px-50">
          <form method="GET" action="{{route('masters.vehicle.int-grade.index')}}" style="margin-bottom:0px !important;">
            <div class="row">
        ...
        <!-- Basic Tables start -->
          ...
          @can('main-navigation-masters-vehicle-int-grade') 
                @if(count($data) > 0 )
                    @if (Auth::guard('web')->user()->can('main-navigation-masters-vehicle-int-grade-delete'))		
                        {{ $data->onEachSide(1)->links('vendor.pagination.bootstrap-4',['multiple_delete_url' => route('masters.vehicle.int-grade.delete-multiple') ] ) }}  
                    @else
