   ...
   <!-- filter  -->
    <div class="card">
        ...
        <div class="card-body pt-75 pb-75 px-50">
          <form method="GET" action="{{route('masters.vehicle.accessories.index')}}" style="margin-bottom:0px !important;">
            <div class="row">
                ...
                <div class="col-md-12 pt-0 text-center">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.vehicle.accessories.index')}}" />
                </div>
            </div>
          </form>
        </div>
        ...
        $permission_and_urls = [
                'multiple_delete' => ['url' => route('masters.vehicle.accessories.delete-multiple') ,
                                    'permission' => 'masters-vehicle-accessories-delete'],

                'multiple_update' =>   ['url' => route('masters.vehicle.accessories.update-multiple') ,
                                     'permission' => 'masters-vehicle-accessories-update']                               
            ];                          
        @endphp
        ...
        <!-- Basic Tables start -->
          ...
          @can('masters-vehicle-accessories') 
                @if(count($data) > 0 )
            ...
            <x-admin.table.table-heading :headingFields="$heading_array"/>                
                ...
                @foreach($data as $item)
                    @include('content.admin.masters.vechiles.accessories.item-tr', ['item'=>$item])    
                        @if(true || request()->input('search.parentOnlyShowAll') == 1)
                        @foreach($item->children as $childItem)
                            @include('content.admin.masters.vechiles.accessories.item-tr', ['item'=>$childItem])    
                        @endforeach                                        
                        @endif
                @endforeach      
