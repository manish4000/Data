...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_30 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.accessories.title')}}</div>
    ...
    <div class="make_col width_20 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        ...
        @php echo  str_ireplace( request()->input('search.keyword'), '<span class="heighlight-string" >'. request()->input('search.keyword').'</span>',isset($item->accessoriesGroup->name) ? $item->accessoriesGroup->name :'' ) @endphp
    </div>
