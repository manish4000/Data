  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="GET" action="{{route('masters.vehicle.subtype.index')}}">
            <div class="row">
                ...
                <div class="col-sm-4 col-md-5 col-lg-3 col-xl-5">
                    <div class="form-group">
            ...
            <div class="row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-4 col-xl-4 pt-1">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.vehicle.subtype.index')}}" />
                </div>
                <div class="col-sm-6 col-md-6 col-lg-4 col-xl-4"></div>
            </div>
          </form>
            ...
            @can('masters-vehicle-subtype') 
                @if(count($data) > 0 )
                    <div class="table-responsive">
                        ...
                        @can('masters-vehicle-subtype-delete')
                            <div class="px-2 my-2">
                                {{-- deleteMultiple() for delete multiple data pass url here  --}}
                                <x-admin.form.buttons.multipleDelete url="{{route('masters.vehicle.subtype.delete-multiple')}}" />
                            </div>
                        @endcan
                        <table class="table" id="master-list">
                            <thead>
                                <tr>
                                        ...
                                        <th class="position-for-filter-heading" data-toggle="tooltip" title="{{__('webCaption.subtype.caption')}}"> {{__('webCaption.subtype.title')}}<x-admin.filter.order-by-filter-div orderBy="name" />
                                        </th>
                            ...
                            <tbody>
                                @foreach($data as $item)
                                    @include('content.admin.masters.vechiles.subtype.item-tr', ['item'=>$item])    
                                    @if( true || request()->input('search.parentOnlyShowAll') == 1)
                                        @foreach($item->children as $childItem)
                                            @include('content.admin.masters.vechiles.subtype.item-tr', ['item'=>$childItem])    
                                        @endforeach 
                                    @endif
                                @endforeach   
