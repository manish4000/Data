...
@section('title', __('webCaption.subtype.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.subtype.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.vehicle.subtype.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.subtype.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.subtype.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.select  tooltip="{{__('webCaption.type.caption')}}"  label="{{__('webCaption.type.title')}}"  id="type_id" for="type_id" name="type_id" placeholder="{{__('webCaption.type.title')}}"  required="required" :optionData="$alltypes" editSelected="{{(isset($data->type_id) && ($data->type_id != null)) ? $data->type_id :''; }}" />
          </div>
        </div>
        
        <div class="col-lg-3 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.subtype.caption')}}" label="{{__('webCaption.subtype.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.subtype.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
