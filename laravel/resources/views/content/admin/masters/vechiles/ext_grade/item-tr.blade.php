...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_45 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.extgrade.title')}}</div>
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.vehicle.ext-grade.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.vehicle.ext-grade.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        @can('main-navigation-masters-vehicle-ext-grade-edit')
        <x-admin.form.buttons.edit href="{{ route('masters.vehicle.ext-grade.edit', $item->id) }}" />
        @endcan
        &nbsp;

        {{-- pass in  deleteSingleData(id , name ,url ) for delete  --}}

        @can('main-navigation-masters-vehicle-ext-grade-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.vehicle.ext-grade.delete')}}" action="{{route('masters.vehicle.ext-grade.delete',$item->id)}}" /> 
        @endcan
    </div>           
</div>    