...
@section('title', __('webCaption.currency.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.currency.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.currency.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.currency.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.currency.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.currency.caption')}}" label="{{__('webCaption.currency.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.currency.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
        ...
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.currency_symbol.caption')}}" label="{{__('webCaption.currency_symbol.title')}}" maxlength="10" for="currency_symbol"   name="currency_symbol"  placeholder="{{ __('webCaption.currency_symbol.title') }}" value="{{old('currency_symbol', isset($data->currency_symbol)?$data->currency_symbol:'' )}}"  required="required" />
          </div>    
        </div>

        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.title.caption')}}" label="{{__('webCaption.title.title')}}" maxlength="100" for="title"   name="title"  placeholder="{{ __('webCaption.title.title') }}" value="{{old('title', isset($data->title)?$data->title:'' )}}"  required="required" />
          </div>    
        </div>
