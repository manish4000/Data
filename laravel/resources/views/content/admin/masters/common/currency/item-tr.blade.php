...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_20 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.currency.title')}}</div>
    ...
    <div class="make_col width_10 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.currency_symbol.title')}}</div>
        @php echo  str_ireplace( request()->input('search.keyword'), '<span class="heighlight-string" >'. request()->input('search.keyword').'</span>',$item->currency_symbol) @endphp
    </div>

    <div class="make_col width_15 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.title.title')}}</div>
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.currency.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.currency.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        @can('masters-currency-edit')
        <x-admin.form.buttons.edit href="{{ route('masters.currency.edit', $item->id) }}" />
        @endcan
        &nbsp;
        ...
        @can('masters-currency-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.currency.delete')}}" action="{{route('masters.currency.delete',$item->id)}}" /> 
        @endcan
    </div>           
</div>    