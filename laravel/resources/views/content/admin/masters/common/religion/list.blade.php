  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="GET" action="{{route('common.religion.index')}}">
            <div class="row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-4 col-xl-3 pt-1 ">
                    ...
                    <x-admin.form.buttons.reset href="{{route('common.religion.index')}}" />
                </div>
            </div>
          </form>
        </div>
            ...
            @can('common-religion') 
                @if(count($data) > 0 )
                    <div class="table-responsive">
                        ...
                        @can('common-religion-delete')
                            <div class="px-2 my-2">
                                {{-- deleteMultiple() for delete multiple data pass url here  --}}
                                <x-admin.form.buttons.multipleDelete url="{{route('common.religion.delete-multiple')}}" />
                            </div>
                        @endcan
                        <table class="table" id="master-list">
                            <thead>
                                <tr>
                                        ...
                                        <th class="position-for-filter-heading" data-toggle="tooltip" title="{{__('webCaption.religion.caption')}}"> {{__('webCaption.religion.title')}}<x-admin.filter.order-by-filter-div orderBy="name" />
                                        </th>
                            ...
                            <tbody>
                                @foreach($data as $item)
                                    @include('content.admin.masters.common.religion.item-tr', ['item'=>$item])    
                                    @if( true || request()->input('search.parentOnlyShowAll') == 1)
                                        @foreach($item->children as $childItem)
                                            @include('content.admin.masters.common.religion.item-tr', ['item'=>$childItem])    
                                        @endforeach 
                                    @endif
                                @endforeach   
