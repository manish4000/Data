...
@section('title', __('webCaption.edit_state.title') )
@else
@section('title', __('webCaption.add_state.title'))
@endif


@section('content')
<form action="{{ route('common.state.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id))  {{__('webCaption.edit_state.title')}}  @else {{__('webCaption.add_state.title')}} @endif 
			</h4>  
		</div>
		<hr class="m-0 p-0">
    ...
    <div class="card-body">
      <div class="row">
          ...
          <div class="col-md-4">
            <div class="form-group">
               <x-admin.form.inputs.select label="{{__('webCaption.country.title')}}"  tooltip="{{__('webCaption.country.caption')}}"           for="country_id" name="country_id"   placeholder="{{ __('locale.country.caption') }}" customClass="country"  editSelected="{{(isset($data->country_id) && ($data->country_id != null)) ? $data->country_id :'' }}"  required="required" :optionData="$countries" />
            </div>
         </div>
         <div class="col-md-4">
            <div class="form-group">
