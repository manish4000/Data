...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_45 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.nationality.title')}}</div>
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.nationality.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.nationality.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        @can('masters-nationality-edit')
        <x-admin.form.buttons.edit href="{{ route('masters.nationality.edit', $item->id) }}" />
        @endcan
        &nbsp;

        {{--  pass in  deleteSingleData(id , name ,url ) for delete  --}}

        @can('masters-nationality-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.nationality.delete')}}" action="{{route('masters.nationality.delete',$item->id)}}" /> 
        @endcan
    </div>           
</div>    