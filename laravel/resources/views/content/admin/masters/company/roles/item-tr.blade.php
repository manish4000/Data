    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.roles.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.roles.update-status')}}" 
               value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
        </td>
        <td>
            @can('masters-company-roles-edit')
             <x-admin.form.buttons.edit href="{{ route('masters.company.roles.edit', $item->id) }}" />
            @endcan
            &nbsp;
