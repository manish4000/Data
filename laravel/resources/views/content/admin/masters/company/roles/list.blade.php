  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="GET" action="{{route('masters.company.roles.index')}}">
            <div class="row">
                ...
                <div class="col-md-12 pt-1 text-center">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.company.roles.index')}}" />
                </div>
            </div>
          </form>
        </div>
                    ...
                    <div class="table-responsive">
                        ...
                        @can('masters-company-roles-delete')
                             <div class="px-2 my-2">
                                ...
                                <x-admin.form.buttons.multipleDelete url="{{route('masters.company.roles.delete-multiple')}}" />
                            </div> 
                        @endcan
                        ...
                        <table class="table" id="master-list">
                            <thead>
                                <tr>
                                        ...
                                        <th class="position-for-filter-heading" data-toggle="tooltip" title="{{__('webCaption.roles.caption')}}"> {{__('webCaption.roles.title')}}<x-admin.filter.order-by-filter-div orderBy="name" />
                                        </th>
                            ...
                            <tbody>
                                @foreach($data as $item)
                                    @include('content.admin.masters.company.roles.item-tr', ['item'=>$item])    
                                    @if( true || request()->input('search.parentOnlyShowAll') == 1)
                                        @foreach($item->children as $childItem)
                                            @include('content.admin.masters.company.roles.item-tr', ['item'=>$childItem])    
                                        @endforeach 
                                    @endif
                                @endforeach   
