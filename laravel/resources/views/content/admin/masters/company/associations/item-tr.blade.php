    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.association.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.association.update-status')}}" 
        ...
        <td>
           ...
           @can('masters-company-association-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.company.association.delete')}}" action="{{route('masters.company.association.delete',$item->id)}}" /> 
           @endcan
        </td>
    </tr>