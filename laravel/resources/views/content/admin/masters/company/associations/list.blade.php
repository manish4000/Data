  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        ...
        <!-- Basic Tables start -->
          <div class="card-body">
            @can('masters-company-association') 
                @if(count($data) > 0 )
                    <div class="table-responsive">
                        ...
                        @can('masters-company-association-delete')
                            <div class="px-2 my-2">
                                ...
                                <x-admin.form.buttons.multipleDelete url="{{route('masters.company.association.delete-multiple')}}" />
                            </div>
                        @endcan
                        <table class="table" id="master-list">
                            ...
                            <tbody>
                                @foreach($data as $item)
                                    @include('content.admin.masters.company.associations.item-tr', ['item'=>$item])    
                                    @if( true || request()->input('search.parentOnlyShowAll') == 1)
                                        @foreach($item->children as $childItem)
                                            @include('content.admin.masters.company.associations.item-tr', ['item'=>$childItem])    
                                        @endforeach 
                                    @endif
                                @endforeach   
