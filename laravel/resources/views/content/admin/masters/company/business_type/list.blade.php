...
<section>
    ...
    <div class="card">
        ...
        <div class="card-body pt-75 pb-75 px-50">
          <form method="GET" action="{{route('masters.company.business-type.index')}}" style="margin-bottom:0px !important;">
            <div class="row">
                ...
                <div class="col-md-12 pt-0 text-center">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.company.business-type.index')}}" />
                </div>
            </div>
          </form>
        </div>
       ...
       @php
        ...
        $permission_and_urls = [
                'multiple_delete' => ['url' => route('masters.company.business-type.delete-multiple') ,
                                    'permission' => 'masters-company-business-type-delete'],

                'multiple_update' =>   ['url' => route('masters.company.business-type.update-multiple') ,
                                     'permission' => 'masters-company-business-type-update']                               
            ];                          
        @endphp
          ...
          @can('masters-company-business-type') 
            ...
            <x-admin.table.table-heading :headingFields="$heading_array"/>                
                ...
                @foreach($data as $item)
                    @include('content.admin.masters.company.business_type.item-tr', ['item'=>$item])    
                        @if(true || request()->input('search.parentOnlyShowAll') == 1)
                        @foreach($item->children as $childItem)
                            @include('content.admin.masters.company.business_type.item-tr', ['item'=>$childItem])    
                        @endforeach                                        
                        @endif
                @endforeach      
