    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td @if($childTdColor != '') class="{{$childTdColor}}" @endif>
             @php echo  str_ireplace( request()->input('search.keyword'), '<span class="heighlight-string" >'. request()->input('search.keyword').'</span>',$item->language_text) @endphp
        </td>
        <td>
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.support-languages.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.support-languages.update-status')}}" 
               value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
        </td>
        <td>
            @can('masters-company-support-languages-edit')
             <x-admin.form.buttons.edit href="{{ route('masters.company.support-languages.edit', $item->id) }}" />
            @endcan
            &nbsp;
           ...
           @can('masters-company-support-languages-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.company.support-languages.delete')}}" action="{{route('masters.company.support-languages.delete',$item->id)}}" /> 
           @endcan
        </td>
    </tr>