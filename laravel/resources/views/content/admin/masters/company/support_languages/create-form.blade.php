...
@section('title', __('webCaption.support_languages.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.support_languages.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.company.support-languages.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.support_languages.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.support_languages.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.support_languages.caption')}}" label="{{__('webCaption.support_languages.title')}}" maxlength="50" for="name" name="name"  placeholder="{{ __('webCaption.support_languages.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
        ...
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.language_text.caption')}}" label="{{__('webCaption.language_text.title')}}" maxlength="50" for="language_text" name="language_text"  placeholder="{{ __('webCaption.language_text.title') }}" value="{{old('language_text', isset($data->language_text)?$data->language_text:'' )}}"  required=""/>
          </div>    
        </div>
