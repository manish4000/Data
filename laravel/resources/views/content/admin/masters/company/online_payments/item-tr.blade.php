    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.online-payments.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.online-payments.update-status')}}" 
               value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
        </td>
        <td>
            @can('masters-company-online-payments-edit')
             <x-admin.form.buttons.edit href="{{ route('masters.company.online-payments.edit', $item->id) }}" />
            @endcan
            &nbsp;

           {{-- pass in  deleteSingleData(id , name ,url ) for delete  --}}

           @can('masters-company-online-payments-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.company.online-payments.delete')}}" action="{{route('masters.company.online-payments.delete',$item->id)}}" /> 
           @endcan
        </td>
    </tr>