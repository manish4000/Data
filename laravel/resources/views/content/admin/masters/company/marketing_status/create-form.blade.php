...
@section('title', __('webCaption.marketingstatus.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.marketingstatus.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.company.marketing-status.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.marketingstatus.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.marketingstatus.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.marketingstatus.caption')}}" label="{{__('webCaption.marketingstatus.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.marketingstatus.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
