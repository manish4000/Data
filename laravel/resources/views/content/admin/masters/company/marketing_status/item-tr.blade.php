...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_45 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.marketingstatus.title')}}</div>
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.marketing-status.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.marketing-status.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        @can('masters-company-marketing-status-edit') 
        <x-admin.form.buttons.edit href="{{ route('masters.company.marketing-status.edit', $item->id) }}" /> 
        @endcan
        &nbsp;
        ...
        @can('masters-company-marketing-status-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.company.marketing-status.delete')}}" action="{{route('masters.company.marketing-status.delete',$item->id)}}" /> 
        @endcan
    </div>           
</div>    