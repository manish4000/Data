...
@section('title', __('webCaption.messenger.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.messenger.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.company.messenger.store')}}" method="POST" enctype="multipart/form-data">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.messenger.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.messenger.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.messenger.caption')}}" label="{{__('webCaption.messenger.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.messenger.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
        ...
        <div class="col-lg-6 col-md-6">
          <div class="form-group">
              @php
                $editImageUrl = (isset($data->logo)) ? "master/company/messenger/".$data->logo : '';
              @endphp
