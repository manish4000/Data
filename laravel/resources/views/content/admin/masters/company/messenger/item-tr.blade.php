    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>
            @if(!empty($item->logo) && isset($item->logo) && is_file(public_path('master').'/company/messenger/'.$item->logo))
            <img src="{{asset('master')}}/company/messenger/{{$item->logo}}" alt="logo" class="rounded" id="logo-preview" width="60"
                height="60">
            @else
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"
                onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.messenger.update-status')}}')"
                dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.messenger.update-status')}}"
                value="{{$item->id}}" checked="{{$displayStatusChecked}}" />
        </td>
        <td>
            @can('masters-company-messenger-edit')
            <x-admin.form.buttons.edit href="{{ route('masters.company.messenger.edit', $item->id) }}" />
            @endcan
            &nbsp;
            ...
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}"
                url="{{route('masters.company.messenger.delete')}}"
                action="{{route('masters.company.messenger.delete',$item->id)}}" />
            @endcan
        </td>
    </tr>