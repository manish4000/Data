...
@section('title', __('webCaption.deals_in.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.deals_in.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.company.deals-in.store')}}" method="POST">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.deals_in.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.deals_in.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.deals_in.caption')}}" label="{{__('webCaption.deals_in.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.deals_in.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
