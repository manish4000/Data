...
@section('title', __('webCaption.bank.title'). ' ' .__('webCaption.edit.title')  )
@else
@section('title', __('webCaption.bank.title').' '. __('webCaption.add.title') )
@endif

@section('content')
<form action="{{ route('masters.company.bank.store')}}" method="POST" enctype="multipart/form-data">
@csrf
<section >
  <div class="card">
    <div class="card-header">
			...
			@if(isset($data->id) && !empty($data->id))
			{{__('webCaption.bank.title'). ' ' .__('webCaption.edit.title')  }}
			@else
			{{  __('webCaption.bank.title').' '. __('webCaption.add.title')}}
			@endif
			</h4>  
		</div>
		<hr class="m-0 p-0">
      ...
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="form-group">
            <x-admin.form.inputs.text tooltip="{{__('webCaption.bank.caption')}}" label="{{__('webCaption.bank.title')}}" maxlength="50" for="name"   name="name"  placeholder="{{ __('webCaption.bank.title') }}" value="{{old('name', isset($data->name)?$data->name:'' )}}"  required="required" />
          </div>    
        </div>
        ...
        <div class="col-lg-6 col-md-6">
          <div class="form-group">
              @php
                $editImageUrl = (isset($data->logo)) ? "master/company/bank/".$data->logo : '';
              @endphp
            <x-admin.form.inputs.file id="" caption="{{__('webCaption.bank.title').'  '.__('webCaption.logo.title')}}" editImageUrl="{{$editImageUrl}}" ImageId="logo-preview" for="logo"  name="logo"  maxFileSize="5000"  placeholder="{{__('webCaption.logo.title')}}" required="" />     
          </div>    
        </div>
