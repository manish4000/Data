    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>{{ (isset($item->country) && !empty($item->country))  ? $item->country : '' }}</td>
        <td>
            @if(!empty($item->logo) && isset($item->logo) && is_file(public_path('master').'/company/bank/'.$item->logo))
            <img src="{{asset('master')}}/company/bank/{{$item->logo}}" alt="logo" class="rounded" id="logo-preview" width="60"
                height="60">
            @else
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"
                onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.bank.update-status')}}')"
                dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.bank.update-status')}}"
                value="{{$item->id}}" checked="{{$displayStatusChecked}}" />
        </td>
        <td>
            @can('masters-company-bank-edit')
            <x-admin.form.buttons.edit href="{{ route('masters.company.bank.edit', $item->id) }}" />
            @endcan
            &nbsp;
            ...
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}"
                url="{{route('masters.company.bank.delete')}}"
                action="{{route('masters.company.bank.delete',$item->id)}}" />
            @endcan
        </td>
    </tr>