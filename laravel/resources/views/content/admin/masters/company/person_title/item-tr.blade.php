    ...
    <tr class="parent-id-{{$item->parent_id}} {{$display}}">
        ...
        <td>
            ...
            <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.company.person-title.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.company.person-title.update-status')}}" 
               value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
        </td>
        <td>
            @can('masters-company-person-title-edit')
             <x-admin.form.buttons.edit href="{{ route('masters.company.person-title.edit', $item->id) }}" />
            @endcan
            &nbsp;
           ...
           @can('masters-company-person-title-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.company.person-title.delete')}}" action="{{route('masters.company.person-title.delete',$item->id)}}" /> 
           @endcan
        </td>
    </tr>