...
<div class="table_row parent-id-{{$item->parent_id}} " style="{{$display}}">
    ...
    <div class="make_col width_45 xs_width_50 @if($childTdColor != '') {{$childTdColor}}  @endif" >
        <div class="div-mobile">{{__('webCaption.designation.title')}}</div>
    ...
    <div class="make_col width_13 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.user.designation.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.user.designation.update-status')}}" 
    ...
    <div class="make_col width_12 xs_width_50 text-xl-center text-lg-center text-md-center">
        ...
        @can('masters-user-designation-edit')
        <x-admin.form.buttons.edit href="{{ route('masters.user.designation.edit', $item->id) }}" />
        @endcan
        &nbsp;
        ...
        @can('masters-user-designation-delete')
            <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.user.designation.delete')}}" action="{{route('masters.user.designation.delete',$item->id)}}" /> 
        @endcan
    </div>           
</div>    