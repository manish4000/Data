@php
   ...
   <tr class="parent-id-{{$item->parent_id}} {{$display}}">
       ...
       <td>
           ...
           <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.billing.add-ons.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.billing.add-ons.update-status')}}" 
              value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
       </td>
       <td>
           @can('masters-billing-add-ons-edit')
            <x-admin.form.buttons.edit href="{{ route('masters.billing.add-ons.edit', $item->id) }}" />
           @endcan
           &nbsp;
          ...
          @can('masters-billing-add-ons-delete')
           <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.billing.add-ons.delete')}}" action="{{route('masters.billing.add-ons.delete',$item->id)}}" /> 
          @endcan
       </td>
   </tr>