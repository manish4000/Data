  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="GET" action="{{route('masters.billing.add-ons.index')}}">
            <div class="row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-4 col-xl-3 pt-1 ">
                    ...
                    <x-admin.form.buttons.reset href="{{route('masters.billing.add-ons.index')}}" />
                </div>
            </div>
          </form>
        </div>
        ...
        <!-- Basic Tables start -->
          <div class="card-body">
            @can('masters-billing-add-ons') 
                @if(count($data) > 0 )
                    <div class="table-responsive">
                        <div class="mt-2 mb-1">
                            {{ $data->onEachSide(1)->appends(request()->query())->links('vendor.pagination.bootstrap-4') }}       
                        </div>
                        @can('masters-billing-add-ons-delete')
                             <div class="px-2 my-2">
                                {{-- deleteMultiple() for delete multiple data pass url here  --}}
                                <x-admin.form.buttons.multipleDelete url="{{route('masters.billing.add-ons.delete-multiple')}}" />
                            </div> 
                        @endcan
                        ...
                        <table class="table" id="master-list">
                            <thead>
                                <tr>
                                        ...
                                        <th class="position-for-filter-heading" data-toggle="tooltip" title="{{__('webCaption.add_ons.caption')}}"> {{__('webCaption.add_ons.title')}}<x-admin.filter.order-by-filter-div orderBy="name" />
                                        </th>
                            ...
                            <tbody>
                                @foreach($data as $item)
                                    @include('content.admin.masters.billing.add_ons.item-tr', ['item'=>$item])    
                                    @if( true || request()->input('search.parentOnlyShowAll') == 1)
                                        @foreach($item->children as $childItem)
                                            @include('content.admin.masters.billing.add_ons.item-tr', ['item'=>$childItem])    
                                        @endforeach 
                                    @endif
                                @endforeach   
