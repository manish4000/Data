@php
   ...
   <tr class="parent-id-{{$item->parent_id}} {{$display}}">
       ...
       <td>
           ...
           <x-admin.form.inputs.listing_checkbox id="list{{$item->id}}"  onclick="changeDisplayStatus('{{$item->id}}','{{route('masters.billing.payment-mode.update-status')}}')"  dataItemId="{{$item->id}}" dataUrl="{{route('masters.billing.payment-mode.update-status')}}" 
              value="{{$item->id}}" checked="{{$displayStatusChecked}}"  /> 
       </td>
       <td>
           @can('masters-billing-payment-mode-edit')
            <x-admin.form.buttons.edit href="{{ route('masters.billing.payment-mode.edit', $item->id) }}" />
           @endcan
           &nbsp;
          ...
          @can('masters-billing-payment-mode-delete')
           <x-admin.form.buttons.delete id="{{$item->id}}" name="{{$item->name}}" url="{{route('masters.billing.payment-mode.delete')}}" action="{{route('masters.billing.payment-mode.delete',$item->id)}}" /> 
          @endcan
       </td>
   </tr>