    ...
    dtUserTable.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $checked = '';
            if(full['is_service'] == 'Yes')  $checked = ' checked="checked" ';
            var $row_output = '<div class="form-group"><div class="custom-control custom-control-success custom-checkbox">' + 
              '<input class="custom-control-input" style="opacity:1 !important;" type="checkbox" onclick="___updateServiceStatus('+full['id']+');" value="'+full['id']+'" id="checkbox' + full['id'] +'" '+$checked+'/>'+
              '</div></div>';
              return $row_output;
          }
        },
