...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    var checked = "";
    var checkedClass = "";
    if (typeof(selPermission) == 'object' && selPermission != null &&  selPermission.includes(sourceItem.slug)) {
      checked = "checked";
      checkedClass = "checkedBackground";
    }

    if ( typeof(selPermission) == 'number' && selPermission == sourceItem.id ) {
      checked = "checked";
      checkedClass = "checkedBackground";
    }
    ...
    if (isThereSubs)
      itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse || collapse ? '<i class="fa fa-plus" style="font-size: 10px"></i>' : '<i class="fa fa-minus" style="font-size: 10px"></i>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';

    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass +' '+ checkedClass + '">' + (isSelectable ? '<input type="checkbox" value="'+sourceItem.slug +'" name="permissions[]" '+checked+' />' : '') + sourceItem.title + '</span>';
    else
      itemHtml += '<label style="font-weight: 500; margin-bottom: 0px"><span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass +' '+ checkedClass + '">' + (isSelectable ? '<input type="radio" value="'+sourceItem.id +'" name="permission_id" style="visibility: hidden; margin-left: -10px" '+checked+' />' : '') + sourceItem.title + '</span></label>';
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined){
      if ($(subMenu).is(':visible'))
        $(item).children('span.comboTreeParentPlus').html('<i class="fa fa-plus" style="font-size: 10px"></i>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
        $(item).children('span.comboTreeParentPlus').html('<i class="fa fa-minus" style="font-size: 10px"></i>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
